...
//Confirmed this -D is automatic when runing em++
...
public:
  ...
  void readbytes(size_t num_elements, void* __out_ buf, int32_t elementSize) {
    ...
    if (readSize > byteSize) {
      BRThrowException("Tried to read " + (uint64_t)readSize + " from Gpu Buffer with size of " + (uint64_t)getByteSize() + ".");
    }
    OglErr::chkErrDbg();
  ...
  void copyDataClientServer(size_t num_elements, const void* frags, int32_t iElementSize) {
    ...
    bindBuffer();
    {
      ...
      if (copySizeBytes > byteSize) {
        BRLogError("Copy size " + (uint64_t)copySizeBytes + " (" + (uint64_t)num_elements + " elements) was larger than the buffer size " + (uint64_t)byteSize + ". Make sure the input size is not -1 or less than 0.");
        unbindBuffer();
        return;
        Gu::debugBreak();
      }
