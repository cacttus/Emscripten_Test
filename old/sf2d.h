...
public:
  ...
  void readbytes(size_t num_elements, void* __out_ buf, int32_t elementSize) {
    ...
    if (readSize > byteSize) {
      BRThrowException("Tried to read " + (int64_t)readSize + " from Gpu Buffer with size of " + (int64_t)getByteSize() + ".");
    }
    OglErr::chkErrDbg();
  ...
  void copyDataClientServer(size_t num_elements, const void* frags, int32_t iElementSize) {
    ...
    bindBuffer();
    {
      ...
      if (copySizeBytes > byteSize) {
        BRLogError("Copy size " + (int64_t)copySizeBytes + " (" + (int64_t)num_elements + " elements) was larger than the buffer size " + (int64_t)byteSize + ". Make sure the input size is not -1 or less than 0.");
        unbindBuffer();
        return;
        Gu::debugBreak();
      }
